...
class User {
    constructor(fullName, email, password, id, balance = 0, expenseItems = [], actions = []) {
    ...
    addExpense(item, amount) {
        if (isNaN(amount)) {
            ...
            const index = expenseItems.findIndex(expenseItem => expenseItem.name === item)
    ...
    deleteExpense(item) {
        const index = this.expenseItems.findIndex(expenseItem => expenseItem.name === item)
...
users = users.map(user => {
    return new User(user.fullName, user.email, user.password, user.id, user.balance, user.expenseItems, user.actions)
})
